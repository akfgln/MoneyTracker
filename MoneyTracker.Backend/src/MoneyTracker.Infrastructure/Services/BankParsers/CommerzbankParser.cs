...
public class CommerzbankParser : IBankFormatParser
{
    public string BankName => "Commerzbank";
    ...
    public async Task<BankStatementInfo> ExtractStatementInfoAsync(string text)
    {
        ...
        // Commerzbank specific patterns
        var accountPattern = @"Konto-Nr\.\s*(\d+)";
        var accountMatch = Regex.Match(text, accountPattern);
    ...
    private ExtractedTransactionDto? ParseMatch(Match match)
    {
        try
        {
